...
class CategoryDetailFragment : Fragment() {
    ...
    // 세 개의 Adapter :topSellingSectionAdapter, CategoryPromotionAdapter, CategorySectionTitleAdapter를 연결해준다.
    private fun setListAdapter() {
        val topSellingSectionAdapter = CategoryTopSellingSectionAdapter()
        val titleAdapter = CategorySectionTitleAdapter()
        val promotionAdapter = CategoryPromotionAdapter()
        // rvCategoryDetail(fragment_category_detail레이아웃)에 세 개의 Adapter를 할당.
        binding.rvCategoryDetail.adapter = ConcatAdapter(topSellingSectionAdapter, titleAdapter, promotionAdapter)

        // topSelling데이터가 업데이트되면 topSellingSectionAdapter에 알려준다. 그때 list타입으로 전달.
